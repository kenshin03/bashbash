...
- (id)init {
	...
	// main view end
	UIImage* twitterLogoImage =  [UIImage imageNamed:@"twitter_logo.png"];
	UIImageView* tmpView = [[UIImageView alloc] initWithImage:twitterLogoImage];
	tmpView.frame = CGRectMake(30, 20, 224, 55);
	...
	[cancelButton addTarget:self action:@selector(cancelTwitterInfoButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)confirmTwitterInfoButtonClicked:(id)sender{
	...
	[LocalStorageManager setObject:loginTextField.text forKey:TWITTERUSER];
	[LocalStorageManager setObject:passwordTextField.text forKey:TWITTERPASSWORD];
...
- (void)viewDidLoad 
{
	loginTextField.delegate = self;
	passwordTextField.delegate = self;
	if ([LocalStorageManager stringForKey:TWITTERUSER] != nil){
		loginTextField.text = [LocalStorageManager stringForKey:TWITTERUSER];
		passwordTextField.text = [LocalStorageManager stringForKey:TWITTERPASSWORD];
	}
}
