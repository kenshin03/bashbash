...
-(void)friends{
	isPost = NO;
	// The URL of the Twitter Request we intend to send
	NSURL *url = [NSURL URLWithString:@"http://twitter.com/statuses/friends.xml"];
	[self request:url];
}
...
-(void)statuses_update:(NSString *)tweetString delegate:(id)requestDelegate requestSelector:(SEL)requestSelector; {
	...
	requestBody = [NSString stringWithFormat:@"status=%@&source=%@",tweetStringFull, NSLocalizedString(@"iPhone 拍拍機BashBash!",nil)];
	[self request:url];
}



-(void)getUserProfile:(NSString*)userName delegate:(id)requestDelegate requestSelector:(SEL)requestSelector{
	delegate = requestDelegate;
	callback = requestSelector;
	
	NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://api.twitter.com/1/users/show/%@.json",userName]];
	[self request:url];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	//	NSLog(@"connectionDidFinishLoading: %@", [[NSString alloc]initWithData:receivedData encoding:NSASCIIStringEncoding]);
	//	NSLog(@"connectionDidFinishLoading delegate: %@", self.delegate);
	//	NSLog(@"connectionDidFinishLoading callback: %@", NSStringFromSelector(self.callback));	
	if(self.delegate && self.callback) {
		if([self.delegate respondsToSelector:self.callback]) {
			[self.delegate performSelector:self.callback withObject:receivedData];
		} else {
